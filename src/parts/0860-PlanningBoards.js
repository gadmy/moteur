  ...
  const PlanningBoards = {
    ...
    SEUIL_LOURD: 1.5,
    //  ==================================================================
    //  HEURES SUPPLEMENTAIRES PREVUES (v602)
    //  ==================================================================
    //  Travail d'une journee = fin prevue (estimatedWrap) - convocation
    //  equipe (crewCall) - pause repas quand elle est renseignee. Les heures
    //  sup sont ce qui depasse le SEUIL DU BUDGET (Reglages du budget, « Seuil
    //  heures sup », 8 h par defaut) : un seul chiffre pour la paie et le
    //  plan de travail, sinon ils finiraient par se contredire.
    //  UNE JOURNEE QUI PASSE MINUIT (15:00 -> 03:00) fait 12 h, pas -12 h.
    //  Sans convocation ou sans fin prevue : rien, pas un zero — on ne sait pas.
    _hhmm: (t) => {
        const m = /^(\d{1,2}):(\d{2})/.exec(String(t || ''));
        return m ? Number(m[1]) * 60 + Number(m[2]) : null;
    },
    seuilHS: () => {
        const v = parseFloat(((state.data && state.data.budget) || {}).overtimeThreshold);
        return (isFinite(v) && v > 0) ? v : 8;
    },
    travailJour: (j) => {
        const a = PlanningBoards._hhmm(j && j.crewCall), f = PlanningBoards._hhmm(j && j.estimatedWrap);
        if(a === null || f === null) return null;
        let d = f - a;
        if(d <= 0) d += 1440;
        // Le repas ne se deduit que pour la part qui tombe DANS les horaires :
        // l'appli pose 13:00-14:00 par defaut, qui est hors d'une journee de
        // nuit 15:00 -> 03:00 et ne doit pas lui retirer une heure.
        const ls = PlanningBoards._hhmm(j.lunchStart), le = PlanningBoards._hhmm(j.lunchEnd);
        if(ls !== null && le !== null) {
            let p = le - ls;
            if(p < 0) p += 1440;
            let pause = 0;
            [ls, ls + 1440].forEach(debut => {
                pause += Math.max(0, Math.min(debut + p, a + d) - Math.max(debut, a));
            });
            d -= Math.min(pause, d);
        }
        return d;
    },
    //  Ecart a la journee de reference, en minutes (negatif = journee courte).
    ecartJour: (j) => {
        const t = PlanningBoards.travailJour(j);
        return t === null ? null : t - Math.round(PlanningBoards.seuilHS() * 60);
    },
    heuresSup: (j) => {
        const e = PlanningBoards.ecartJour(j);
        return e === null ? null : Math.max(0, e);
    },
    //  « 3h », « 2h30 », « -1h15 » ; signe force pour la colonne +/-.
    _duree: (min, signe) => {
        if(min === null || min === undefined) return '';
        const neg = min < 0, a = Math.abs(Math.round(min));
        const h = Math.floor(a / 60), m = a % 60;
        const txt = h + 'h' + (m ? String(m).padStart(2, '0') : '');
        return (neg ? '-' : (signe && a ? '+' : '')) + txt;
    },
    //  Les deux colonnes du modele pour toutes les journees d'un coup : les
    //  heures sup du jour, et le « +/- » CUMULE depuis le debut du tournage
    //  (les journees courtes compensent-elles les longues ?). Seuls les jours
    //  de plateau comptent.
    heuresSupParJour: (lignes) => {
        let cumul = 0, connu = false;
        return (lignes || []).map(l => {
            if(!l.plateau) return { hs: '', cumul: '' };
            const e = PlanningBoards.ecartJour(l.jour);
            if(e === null) return { hs: '', cumul: connu ? PlanningBoards._duree(cumul, true) : '' };
            cumul += e; connu = true;
            return { hs: e > 0 ? PlanningBoards._duree(e) : '0', cumul: PlanningBoards._duree(cumul, true) || '0' };
        });
    },
    dureeMoyenne: () => {
    ...
    tableauJournees: () => {
        ...
        const GAUCHE = avecB ? 15 : 14;
        const hsJours = PlanningBoards.heuresSupParJour(lignes);
        const seuilTxt = PlanningBoards._duree(Math.round(PlanningBoards.seuilHS() * 60));
        ...
        let html = '<div class="pdt-bloc" data-bloc="journees"><h3 class="pdt-titre">📆 Plan de travail — journées (modèle horizontal)'
                 ...
                 + '<th title="Heures supplémentaires prévues : au-delà de ' + esc(seuilTxt) + ' de travail (seuil des Réglages du budget), pause repas déduite">H. sup</th>'
                 + '<th title="Écart cumulé à la journée de ' + esc(seuilTxt) + ' depuis le début du tournage">+/-</th>'
        ...
        lignes.forEach((l, iJour) => {
            ...
            html += '<tr class="' + (l.plateau ? 'pdt-plateau' : 'pdt-hors') + '">'
                ...
                + '<td class="pdt-fin">' + esc((j.crewCall || '') + (j.estimatedWrap ? '–' + j.estimatedWrap : '')) + '</td>'
                + (() => {
                ...
                + '<td class="pdt-fin' + (hsJours[iJour].hs && hsJours[iJour].hs !== '0' ? ' est-lourde' : '') + '">' + esc(hsJours[iJour].hs) + '</td>'
                + '<td class="pdt-fin">' + esc(hsJours[iJour].cumul) + '</td>'
    ...
    _modeleJournees: () => {
        ...
        const cols = [
            ...
            { t: 'Effet', w: 9 }, { t: 'Horaires', w: 18 }, { t: 'Min', w: 16 },
            { t: 'H. sup', w: 11 }, { t: '+/-', w: 11 }
