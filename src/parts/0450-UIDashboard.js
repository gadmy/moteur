  ...
  const UIDashboard = {
    ...
    // kind = 'dossier' : on range un DOSSIER (v602, clic droit). Les
    // dossiers ou il ne peut pas aller (lui-meme, ses sous-dossiers) ne sont
    // pas proposes.
    showMoveMenu: (e, id, pos, kind) => {
        e.stopPropagation();
        document.querySelectorAll('.priority-menu').forEach(m => m.remove());
        const dossier = kind === 'dossier';
        const current = dossier ? ((ProjectFolders.get(id) || {}).parentId || null) : ProjectFolders.folderOf(id);
        const items = [{ id: '', name: '🏠 Racine (aucun dossier)', depth: 0, color: '' }];
        const walk = (parentId, depth) => {
            ProjectFolders.children(parentId).forEach(f => {
                if(dossier && !ProjectFolders.accepte(id, f.id)) return;
                items.push({ id: f.id, name: f.name, depth: depth, color: f.color }); walk(f.id, depth + 1);
            });
        };
        walk(null, 0);
        ...
        const action = dossier ? 'app.UIDashboard._rangerDossier' : 'app.UIDashboard.moveToFolder';
        menu.innerHTML = items.map(it => `
            <div class="priority-menu-item" onclick="document.querySelectorAll('.priority-menu').forEach(m => m.remove()); ${action}(${Utils.jsArg(id)}, ${Utils.jsArg(it.id)})">
                <span style="display:inline-block; width:${it.depth * 14}px;"></span>
                ${it.id ? `<span class="dot" style="background:${Utils.escape(it.color || '#9E9E9E')}"></span>` : ''}
                ${Utils.escape(it.name)}${it.id === (current || '') ? ' ✓' : ''}
            </div>`).join('');
        UIDashboard._placerMenu(menu, e, pos);
    },
    ...
    showPriorityMenu: (e, projectId, pos, kind) => {
        e.stopPropagation();
        const dossier = kind === 'dossier';
        ...
        const menu = document.createElement('div');
        menu.className = 'priority-menu';
        const action = dossier ? 'app.UIDashboard.setFolderPriority' : 'app.UIDashboard.setPriority';
        const actuelle = dossier ? ((ProjectFolders.get(projectId) || {}).priority || '')
            : (((UIDashboard._projets || []).find(x => String(x.id) === String(projectId)) || {}).priority || '');
        menu.innerHTML = priorities.map(p => {
            const val = p.label === 'Aucune' ? '' : p.label;
            return `
            <div class="priority-menu-item" onclick="${action}(${Utils.jsArg(projectId)}, ${Utils.jsArg(val)})">
                <span class="dot" style="background:${p.color}"></span>
                ${p.label}${val === actuelle ? ' ✓' : ''}
            </div>`;
        }).join('');
        UIDashboard._placerMenu(menu, e, pos);
    },

    setFolderPriority: (id, priority) => {
        document.querySelectorAll('.priority-menu').forEach(m => m.remove());
        if(!ProjectFolders.update(id, { priority: priority || '' })) return;
        Utils.toast(priority ? `Priorité "${priority}" définie` : 'Priorité supprimée', 'success');
        UIDashboard.renderProjectList();
    },

    // Pose un menu : au pointeur (clic droit, pos) ou sous le bouton clique,
    // toujours entier dans la fenetre, et ferme au clic ailleurs.
    _placerMenu: (menu, e, pos) => {
        menu.style.position = 'fixed';
        document.body.appendChild(menu);
        let x, y;
        if(pos) { x = pos.x; y = pos.y; }
        else { const r = e.target.getBoundingClientRect(); x = r.left; y = r.bottom + 5; }
        x = Math.max(8, Math.min(x, window.innerWidth - menu.offsetWidth - 8));
        y = Math.max(8, Math.min(y, window.innerHeight - menu.offsetHeight - 8));
    ...
    // CLIC DROIT SUR UN PROJET OU UN DOSSIER DU HUB (v602) : priorite,
    // deplacer vers un dossier, supprimer. Memes fonctions que les petits
    // boutons des cartes, donc memes droits : seul le proprietaire met un
    // projet a la corbeille, les autres le quittent (il sort de leur hub).
    menuContextuel: (e, kind, id) => {
        e.preventDefault(); e.stopPropagation();
        document.querySelectorAll('.priority-menu').forEach(m => m.remove());
        let pos = { x: e.clientX, y: e.clientY };
        // Touche « menu » du clavier : pas de pointeur, on se pose sur la tuile.
        if(!e.clientX && !e.clientY && e.currentTarget && e.currentTarget.getBoundingClientRect) {
            const r = e.currentTarget.getBoundingClientRect(); pos = { x: r.left + 20, y: r.top + 20 };
        }
        UIDashboard._ctxPos = pos;
        const p = kind === 'projet' ? (UIDashboard._projets || []).find(x => String(x.id) === String(id)) : null;
        if(kind === 'projet' && !p) return;
        const suppr = kind === 'dossier' ? '🗑️ Supprimer le dossier'
            : (p.role === 'owner' ? '🗑️ Mettre à la corbeille' : '🚪 Quitter ce projet');
        const fleche = ' <span style="margin-left:auto; opacity:.6;">▸</span>';
        const item = (act, txt) => `<div class="priority-menu-item" role="menuitem" onclick="app.UIDashboard._ctxAction('${kind}', ${Utils.jsArg(id)}, '${act}')">${txt}</div>`;
        const menu = document.createElement('div');
        menu.className = 'priority-menu';
        menu.setAttribute('role', 'menu');
        menu.innerHTML = item('prio', '🏷️ Priorité' + fleche)
            + item('deplacer', '📁 Déplacer vers un dossier' + fleche)
            + item('suppr', suppr);
        UIDashboard._placerMenu(menu, e, pos);
    },
    _ctxAction: (kind, id, act) => {
        document.querySelectorAll('.priority-menu').forEach(m => m.remove());
        const pos = UIDashboard._ctxPos;
        const faux = { stopPropagation: () => {}, target: document.body };
        if(act === 'prio') return UIDashboard.showPriorityMenu(faux, id, pos, kind);
        if(act === 'deplacer') return UIDashboard.showMoveMenu(faux, id, pos, kind);
        if(act !== 'suppr') return;
        if(kind === 'dossier') return UIDashboard.deleteFolder(id);
        const p = (UIDashboard._projets || []).find(x => String(x.id) === String(id));
        if(!p) return;
        if(p.role === 'owner') Store.deleteProject(p.id, p.role);
        else Store.leaveProject(p.id, p.role);
    },
    ...
    _renderFolderArea: () => {
        ...
        ProjectFolders.children(cur).forEach(f => {
            ...
            tile.addEventListener('contextmenu', (e) => UIDashboard.menuContextuel(e, 'dossier', f.id));
    ...
    renderProjectList: async () => {
        ...
        const list = await Store.getProjectsList(); 
        UIDashboard._projets = list; // lu par le clic droit (role, priorite)
        ...
        if(list.length > 0) {
            ...
            list.filter(p => (ProjectFolders.folderOf(p.id) || null) === (UIDashboard.currentFolderId || null)).forEach((p, idx) => { 
                ...
                card.addEventListener('contextmenu', (e) => UIDashboard.menuContextuel(e, 'projet', p.id));
